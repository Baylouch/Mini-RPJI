    ...
    private void OnEnable()
    {
        if (Scenes_Control.instance)
        {
            ...
            if (continueButton)
            {
                ...
                continueButton.onClick.AddListener(() => Camera.main.transform.position = playerHealth.transform.position); // Set camera on player
                continueButton.onClick.AddListener(() => Camera.main.transform.SetParent(playerHealth.transform)); // Link camera to the player
                continueButton.onClick.AddListener(() => Camera.main.transform.position = new Vector3(Camera.main.transform.position.x, Camera.main.transform.position.y, - 10)); // Offset and reset camera scale
                continueButton.onClick.AddListener(() => Camera.main.transform.localScale = Vector3.one); // Offset and reset camera scale
